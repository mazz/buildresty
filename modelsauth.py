...
class Activation(Base):
    ...
    aid = Column(Integer, ForeignKey('users.uid'), primary_key=True)
...
class UserMgr(object):
    ...
    def get_list(active=None, order=None, limit=None):
        ...
        if active is not None:
            user_query = user_query.filter(User.is_active == active)
    ...
    def get(user_id=None, username=None, email=None, api_key=None):
        ...
        if user_id is not None:
            return user_query.filter(User.uid == user_id).first()
...
class User(Base):
    ...
    invited_by = Column('invited_by', Unicode(64))
